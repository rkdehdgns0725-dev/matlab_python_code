...
for N=1.5
    ...
    % 밴드별 독립 맥스치 정규화
        ...
        Y_cell = mat2cell(Y_train_pure(:, 1:numChunks_train*chunkSize), 1, chunkSize*ones(1, numChunks_train))';
        
        % --- [검증 데이터] 규격 일치를 위한 청크 셀 배열화 (수정 완료) ---
        numChunks_val = floor(size(X_val_pure, 2) / chunkSize);
        if numChunks_val == 0 % 만약 검증 데이터가 너무 작아 청크가 안 나오면 최소 1개 확보
            X_val_cell = {X_val_pure};
            Y_val_cell = {Y_val_pure};
        else
            X_val_cell = mat2cell(X_val_pure(:, 1:numChunks_val*chunkSize), 8, chunkSize*ones(1, numChunks_val))';
            Y_val_cell = mat2cell(Y_val_pure(:, 1:numChunks_val*chunkSize), 1, chunkSize*ones(1, numChunks_val))';
        end
    
        % --- ValidationFrequency 자동 설정 ---
        % 1 에포크당 총 이터레이션 수 = numChunks_train
        % 에포크당 최소 1~2회 검증이 수행되도록 주기를 대량 유동적으로 고정
        valFrequency = max(1, floor(numChunks_train / 2)); 
        ...
        options = trainingOptions("adam", ... 
            ...
            MaxEpochs = maxEpochs, ...
            miniBatchSize = 1, ... % 🚨 중요: 셀 배열 자체가 청크 단위 묶음이므로 미니배치 사이즈는 1로 두어야 셀 단위 학습이 됩니다.
            GradientThreshold = 1, ...
            Plots="training-progress", ...
            Shuffle = 'every-epoch', ...
            ExecutionEnvironment = "auto", ...
            ValidationData={X_val_cell, Y_val_cell}, ... % 셀 배열 규격 매칭
            ValidationFrequency = valFrequency, ...      % 자동 계산된 이터레이션 주기 적용
    ...
    file_name=['sequence_NNBF_learningdata_N',num2str(N),'version_',num2str(version),'.mat'];
